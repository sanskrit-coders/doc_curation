...
def dump_all_sargas(base_dir):
    for kaanda_index in text_data.get_subunit_list(file_path=unit_info_file, unit_path_list=[]):
        ...
        for sarga_index in sarga_list:
            ...
            url = "https://www.valmiki.iitk.ac.in/sloka?field_kanda_tid=%d&language=dv&field_sarga_value=%d" % (kaanda_index, sarga_index)
...
def dump_commentary(base_dir, commentary_id):
    for kaanda_index in text_data.get_subunit_list(file_path=unit_info_file, unit_path_list=[]):
        ...
        for sarga_index in sarga_list:
            logging.info("Kanda %d Sarga %d", kaanda_index, sarga_index)
            out_path = os.path.join(base_dir, "%d" % kaanda_index, "%03d.md" % sarga_index)
            url = "https://www.valmiki.iitk.ac.in/commentaries?language=dv&field_commnetary_tid=%d&field_kanda_tid=%d&field_sarga_value=%d" % (commentary_id, kaanda_index, sarga_index)
            title_maker = lambda soup, title_prefix : sanscript.transliterate("%03d" % sarga_index, sanscript.IAST, sanscript.DEVANAGARI)
