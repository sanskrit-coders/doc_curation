...
def set_book_content(dry_run=False):
  md_files = doc_curation.md.library.arrangement.get_md_files_from_path(dir_path=dest_dir_suuktas, file_pattern="**/_index.md", file_name_filter=lambda x: regex.match("\\d\\d", os.path.basename(os.path.dirname(x))))
  for md_file in md_files:
    ...
    if os.path.exists(file_path):
      ...
      content += "%s\n" % doc_curation.md.content_processor.include_helper.get_include(field_names=None, classes=None, title="अनुक्रमणी (Whitney)", url=url,
                                                                                       h1_level=2)

    md_file.replace_content_metadata(new_content=content, dry_run=dry_run)
  doc_curation.md.library.arrangement.fix_index_files(dir_path=dest_dir_suuktas)


def set_suukta_content(dry_run=False):
  md_files = doc_curation.md.library.arrangement.get_md_files_from_path(dir_path=dest_dir_suuktas, file_pattern="**/*.md", file_name_filter=lambda x: len(regex.findall("\\d\\d\\d", os.path.basename(x))) > 0)
  for md_file in md_files:
...
def get_suukta_meta_content(suukta_id):
  ...
  if os.path.exists(file_path):
    url = regex.sub(".+?/vedAH/", "/vedAH/", file_path).replace("/static/", "/")
    content += "%s\n" % doc_curation.md.content_processor.include_helper.get_include(field_names=None, classes=None, title="अनुक्रमणी (VH)", url=url, h1_level=3)
  ...
  if os.path.exists(file_path):
    url = regex.sub(".+?/vedAH/", "/vedAH/", file_path).replace("/static/", "/")
    content += "%s\n\n\n" % doc_curation.md.content_processor.include_helper.get_include(field_names=None, classes=None, title="Griffith", url=url,
                                                                                         h1_level=3)
  return content
...
def get_rk_content(rk_file_name, suukta_id):
  ...
  if os.path.exists(file_path):
    url = regex.sub(".+?/vedAH/", "/vedAH/", file_path).replace("/static/", "/")
    content += "%s\n" % doc_curation.md.content_processor.include_helper.get_include(field_names=None, classes=None, title="Whitney", url=url, h1_level=3)
  file_path = os.path.join(dest_dir_static, "griffith", suukta_id, rk_file_name)
  if os.path.exists(file_path):
    url = regex.sub(".+?/vedAH/", "/vedAH/", file_path).replace("/static/", "/")
    content += "%s\n" % doc_curation.md.content_processor.include_helper.get_include(field_names=None, classes=None, title="Griffith", url=url, h1_level=3)
...
def set_suukta_info_to_match(dest_dir, dry_run=False):
  md_files = doc_curation.md.library.arrangement.get_md_files_from_path(dir_path=dest_dir_suukta_info, file_pattern="**/*.md", file_name_filter=lambda x: len(regex.findall("\\d\\d\\d", os.path.basename(x))) > 0)
  for md_file in md_files:
