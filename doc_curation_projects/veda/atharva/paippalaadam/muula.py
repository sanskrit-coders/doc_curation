...
def import_basic(src_dir=paippalaadam.CONTENT_DIR):
  PATTERN_RK = "\n[^#\s<>\[\(][\s\S]+? ॥ *[०-९\d\.]+ *॥\s*?(?=\n|$)"
...
def copy_to_vishvas(dry_run=False):
  content_helper.copy_contents(src_dir=paippalaadam.MULA_DIR, dest_dir=paippalaadam.MULA_DIR.replace("mUlam", "vishvAsa-prastutiH"), detail_title="मूलम् (VS)", dest_content_condition=lambda x: not text_utils.detect_vishvaasa_mods(content=x)[0], dry_run=dry_run)
