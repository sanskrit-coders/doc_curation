...
def dump(dest_dir, suukta_ids, dry_run=False):
  ...
  for suukta_id in suukta_ids:
    ...
    for match in matches:
      ...
      md_file.transform(content_transformer=lambda c, m: details_helper.transform_detail_contents_with_soup(content=c, metadata=m, transformer=details_helper.detail_content_replacer_soup, title="Griffith", new_text=content), dry_run=dry_run)
