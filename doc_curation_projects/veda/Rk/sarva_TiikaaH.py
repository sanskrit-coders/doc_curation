...
def fix_bare_Rk_files():
  ...
  for source_path in source_paths:
    ...
    dest_md_file.transform(content_transformer=lambda c, m: details_helper.insert_adjascent_element(content=c, metadata=m, title="+Jamison Brereton", new_element=details[0]))
...
def update_commentary(commentary_dir, commentary_name, dry_run=False):
  ...
  for dest_path in dest_paths:
    ...
    md_file.transform(content_transformer=lambda c, m: details_helper.transform_details_with_soup(content=c, metadata=m, content_str_transformer=details_helper.detail_content_replacer_soup, title=commentary_name, new_text=content), dry_run=dry_run)
