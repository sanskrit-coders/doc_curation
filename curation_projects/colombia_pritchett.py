...
def title_maker(soup, title_prefix, index):
    if len(soup.select("h1")) > 0:
        title = souper.title_from_element(soup=soup, title_css_selector="h1", title_prefix=title_prefix, index=index)
    else:
        title = souper.title_from_element(soup=soup, title_css_selector="h2", title_prefix=title_prefix, index=index)
    return title

dumper = lambda url, outfile_path, dry_run, index: souper.dump_text_from_element(url=url, outfile_path=outfile_path, text_css_selector="body", title_maker=title_maker, html_fixer=html_fixer, dry_run=dry_run, index=index)
