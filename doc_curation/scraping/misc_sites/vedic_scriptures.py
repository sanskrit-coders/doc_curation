...
def dump_mantra_details(dest_path, url, commentaries_needed, mode=ForceMode.NONE, muula_insertion_mode="all_detail"):
  ...
  if not os.path.exists(dest_path):
    ...
    if pada_paaTha is not None:
      content = details_helper.transform_details_with_soup(content=content, metadata=metadata, content_transformer=details_helper.detail_content_replacer_soup, title="पदपाठः", replacement=pada_paaTha)
