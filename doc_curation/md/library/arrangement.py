...
def shift_details(dir_path, substitute_content_offset, detail_title, start_index=None, end_index=None, index_position=0, dry_run=False):
  ...
  def replacer(md_file, content):
    ...
    if md_detail is None:
      ...
      md_file.transform(content_transformer=lambda c, m: details_helper.transform_detail_contents_with_soup(content=c, metadata=m, transformer=details_helper.detail_content_replacer_soup, title=detail_title, replacement=detail.content), dry_run=dry_run)
